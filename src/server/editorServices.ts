    ...
    while (true) {
        ...
        if (
            ...
            (
                ...
                // TODO: sheetal: Should this flag be shared between trying to load solution for find all references vs trying to find default project
        ...
        const ancestor = project.projectService.findCreateOrReloadConfiguredProject(
            ...
            reason,
            allowDeferredClosed,
            !searchOnlyPotentialSolution ? info.fileName : undefined,
            reloadedProjects,
        ...
        if (
            ...
            Debug.assert(ancestor.project.isInitialLoadPending());
    ...
    function callbackWithProjectFoundUsingFind(child: ConfiguredProject) {
        ...
        switch (kind) {
            case ConfiguredProjectLoadKind.Create:
                sentConfigFileDiag = child.projectService.updateConfiguredProject(child, triggerFile);
                break;
...
/** @internal */
export interface ConfigFileInfoForOpenFile {
    /** config file for info cached */
    config: ConfigFileName | undefined;
    ancestorConfigFiles?: Map<NormalizedPath, ConfigFileName>;
    /** Set of all canonical config file lookups to find default project */
    configsLookedUp: Set<NormalizedPath>;
}
function configFileNameForInfo(info: OpenScriptInfoOrClosedOrConfigFileInfo, value: ConfigFileInfoForOpenFile) {
    if (!isAncestorConfigFileInfo(info)) {
        return value.config;
    }
    else {
        return value.ancestorConfigFiles?.get(info.fileName);
    }
}
export class ProjectService {
    ...
    private readonly configFileInfoForOpenFiles = new Map<Path, ConfigFileInfoForOpenFile>();
    private openFilesRootOfInferredProject = new Set<Path>();
    /**
    ...
    private pendingOpenFileProjectUpdates?: Map<Path, ConfigFileInfoForOpenFile>;
    /** @internal */
    ...
    /** @internal */
    tryGetDefaultProjectForEnsuringConfiguredProjectForFile(fileNameOrScriptInfo: NormalizedPath | ScriptInfo): Project | undefined {
        const scriptInfo = isString(fileNameOrScriptInfo) ? this.getScriptInfoForNormalizedPath(fileNameOrScriptInfo) : fileNameOrScriptInfo;
        if (!scriptInfo) return undefined;
        if (this.pendingOpenFileProjectUpdates?.delete(scriptInfo.path)) {
            this.updateDefaultProjectForPendingOpenFileUpdate(scriptInfo.path);
    ...
    private onConfigFileChanged(configFileName: NormalizedPath, canonicalConfigFilePath: NormalizedPath, eventKind: FileWatcherEventKind) {
        ...
        if (project) updatedProjects.add(project);
        this.openFiles.forEach((_projectRootPath, path) => {
            const pendingConfigFileInfo = this.pendingOpenFileProjectUpdates?.get(path);
            const configFileInfo = this.configFileInfoForOpenFiles.get(path);
            if (!configFileInfo?.configsLookedUp.has(canonicalConfigFilePath) && !pendingConfigFileInfo?.configsLookedUp.has(canonicalConfigFilePath)) return;
            // Invalidate default config file name for open file
            if (configFileInfo) this.configFileInfoForOpenFiles.delete(path);
            const info = this.getScriptInfoForPath(path)!;
            // This tries to search for a tsconfig.json for the given file. If we found it,
            // we first detect if there is already a configured project created for it: if so,
            // we re- read the tsconfig file content and update the project only if we havent already done so
            // otherwise we create a new one.
            const configFileNameForInfo = this.getConfigFileNameForFile(info, /*findFromCacheOnly*/ false);
            if (configFileNameForInfo) {
                const { project, isNew } = this.getNewOrUpdatedConfiguredProject(
                    configFileNameForInfo,
                    `Change in config file ${configFileName} detected, Creating possible configured project for ${info.fileName} to open`,
                    info.fileName,
                    /*delayLoad*/ true,
                );
                if (tryAddToSet(updatedProjects, project)) {
                    if (project.canonicalConfigFilePath === canonicalConfigFilePath) {
                        project.pendingUpdateLevel = ProgramUpdateLevel.Full;
                ...
                if (!this.pendingOpenFileProjectUpdates?.has(path)) {
                    (this.pendingOpenFileProjectUpdates ??= new Map()).set(path, configFileInfo);
                }
            }
        });
        this.delayEnsureProjectForOpenFiles();
    ...
    private closeOpenFile(info: ScriptInfo, skipAssignOrphanScriptInfosToInferredProject?: true) {
        ...
        this.configFileInfoForOpenFiles.delete(info.path);
        this.pendingOpenFileProjectUpdates?.delete(info.path);
        Debug.assert(!this.openFilesRootOfInferredProject.has(info.path));
    ...
    private configFileExists(configFileName: NormalizedPath, canonicalConfigFilePath: NormalizedPath, info: OpenScriptInfoOrClosedOrConfigFileInfo) {
        ...
        if (configFileExistenceInfo) {
            ...
            if (this.addConfigFileNameForOpenFile(canonicalConfigFilePath, info)) {
        ...
        if (this.addConfigFileNameForOpenFile(canonicalConfigFilePath, info)) {
            (openFilesImpactedByConfigFile ??= new Set()).add(info.path);
        }
    ...
    private addConfigFileNameForOpenFile(
        ...
        if (!this.openFiles.has(info.path)) return false;
        if (isAncestorConfigFileInfo(info) && !info.isForDefaultProject) return;
        const configFileInfo = this.configFileInfoForOpenFiles.get(info.path);
        if (configFileInfo) {
            configFileInfo.configsLookedUp.add(canonicalConfigFilePath);
        }
        else {
            this.configFileInfoForOpenFiles.set(info.path, {
                config: undefined,
                configsLookedUp: new Set([canonicalConfigFilePath]),
            });
        }
        return true;
    }
    ...
    private stopWatchingConfigFilesForClosedScriptInfo(info: ScriptInfo) {
        ...
        const isRootOfInferredProject = this.openFilesRootOfInferredProject.delete(info.path);
    ...
    startWatchingConfigFilesForInferredProjectRoot(info: ScriptInfo) {
        if (this.serverMode !== LanguageServiceMode.Semantic) return;
        Debug.assert(info.isScriptOpen());
        this.openFilesRootOfInferredProject.add(info.path);
    ...
    stopWatchingConfigFilesForInferredProjectRoot(info: ScriptInfo) {
        if (this.serverMode !== LanguageServiceMode.Semantic) return;
        if (!this.openFilesRootOfInferredProject.delete(info.path)) return;
        this.forEachConfigFileLocation(info, canonicalConfigFilePath => {
            const configFileExistenceInfo = this.configFileExistenceInfoCache.get(canonicalConfigFilePath);
    ...
    private getConfigFileNameForFileFromCache(
        ...
        if (lookInPendingFilesForValue) {
            const pendingValue = this.pendingOpenFileProjectUpdates?.get(info.path);
            if (pendingValue) {
                const result = configFileNameForInfo(info, pendingValue);
                if (result !== undefined) return result;
            }
        }
        const result = this.configFileInfoForOpenFiles.get(info.path);
        return result ? configFileNameForInfo(info, result) : undefined;
    }

    private setConfigFileNameForFileInCache(info: OpenScriptInfoOrClosedOrConfigFileInfo, configFileName: NormalizedPath | undefined) {
        if (!this.openFiles.has(info.path)) return; // Dont cache for closed script infos
        const configInfo = this.configFileInfoForOpenFiles.get(info.path);
        const result = configFileName || false;
        if (isAncestorConfigFileInfo(info)) {
            Debug.assertIsDefined(configInfo);
            (configInfo.ancestorConfigFiles ??= new Map()).set(info.fileName, configFileName || false);
        }
        else if (configInfo) {
            configInfo.config = result || false;
        }
        else {
            this.configFileInfoForOpenFiles.set(info.path, {
                config: result || false,
                configsLookedUp: new Set(),
            });
        }
    }
    ...
    private createLoadAndUpdateConfiguredProject(configFileName: NormalizedPath, reason: string, triggerFile: NormalizedPath | undefined) {
        ...
        this.updateConfiguredProject(project, triggerFile);
        return project;
    }
    ...
    reloadConfiguredProjectClearingSemanticCache(
        ...
        this.reloadConfiguredProject(project, `User requested reload projects: ${reason}`);
        return true;
    }
    ...
    reloadProjects() {
        ...
        this.configFileInfoForOpenFiles.clear();
            ...
            projects.forEach(project => {
                if (this.getHostPreferences().lazyConfiguredProjectsFromExternalProject) {
                    ...
                    this.reloadConfiguredProjectClearingSemanticCache(
                        project,
                        `Reloading configured project in external project: ${externalProjectName}`,
    ...
    /** @internal */
    updateDefaultProjectForPendingOpenFileUpdate(path: Path) {
        const info = this.getScriptInfoForPath(path)!;
        this.tryFindDefaultConfiguredProjectForOpenScriptInfo(info, ConfiguredProjectLoadKind.Create);
    }
    ...
    private ensureProjectForOpenFiles() {
        ...
        this.pendingOpenFileProjectUpdates = undefined;
        pendingOpenFileProjectUpdates?.forEach((_config, path) => this.updateDefaultProjectForPendingOpenFileUpdate(path));
    ...
    private getNewOrUpdatedConfiguredProject(
        configFileName: NormalizedPath,
        reason: string,
        ...
        if (!project) {
            project = !delayLoad ?
                this.createLoadAndUpdateConfiguredProject(configFileName, reason, triggerFile) :
                this.createConfiguredProject(configFileName, reason);
            sentConfigFileDiag = !delayLoad;
        }
        // Ensure project is ready to check if it contains opened script info
        else if (!delayLoad) {
            sentConfigFileDiag = this.updateConfiguredProject(project, triggerFile);
        }
        return { project, isNew, sentConfigFileDiag };
    }

    /**
     * returns true if configFileDiag was sent
     * @internal
     */
    updateConfiguredProject(project: ConfiguredProject, triggerFile: NormalizedPath | undefined) {
        if (triggerFile) {
    ...
    private getNewOrReloadedConfiguredProject(
        configFileName: NormalizedPath,
        reason: string,
        reloadedProjects: Set<ConfiguredProject>,
        delayReloadedConfiguredProjects: Set<ConfiguredProject> | undefined,
    ): FindCreateOrLoadConfiguredProjectResult {
        let project = this.findConfiguredProjectByProjectName(configFileName);
        const isNew = !project;
        let sentConfigFileDiag = false;
        if (!delayReloadedConfiguredProjects) {
            // Create project
            if (!project) project = this.createConfiguredProject(configFileName, reason);
            sentConfigFileDiag = this.reloadConfiguredProjectClearingSemanticCache(project, reason, reloadedProjects);
        }
        else if (!project) {
            project = this.createConfiguredProject(configFileName, `User requested reload projects: ${reason}`);
            delayReloadedConfiguredProjects.add(project);
        }
        else if (!reloadedProjects.has(project) && !delayReloadedConfiguredProjects.has(project) && !project.isInitialLoadPending()) {
            project.pendingUpdateLevel = ProgramUpdateLevel.Full;
            project.pendingUpdateReason = `User requested reload projects: ${reason}`;
            delayReloadedConfiguredProjects.add(project);
        }
        return { project, isNew, sentConfigFileDiag };
    }
    ...
    findCreateOrReloadConfiguredProject(
        ...
        switch (kind) {
            case ConfiguredProjectLoadKind.Find:
                const project = this.findConfiguredProjectByProjectName(configFileName, allowDeferredClosed);
                return project && { project, isNew: false, sentConfigFileDiag: false };
            case ConfiguredProjectLoadKind.Create:
                return this.getNewOrUpdatedConfiguredProject(configFileName, reason!, triggerFile, !!delayLoad);
            case ConfiguredProjectLoadKind.Reload:
                return this.getNewOrReloadedConfiguredProject(configFileName, reason!, reloadedProjects!, delayReloadedConfiguredProjects);
        }
    }
    ...
    private tryFindDefaultConfiguredProjectForOpenScriptInfo(
        ...
        function isDefaultProject(project: ConfiguredProject): ConfiguredProject | undefined {
            ...
            if (projectContainsInfoDirectly(project, info)) return defaultProject = project;
            // If this project uses the script info, if default project is not found, use this project as possible default
            possiblyDefault ??= project.containsScriptInfo(info) ? project : undefined;
        }
    ...
    private ensureProjectChildren(project: ConfiguredProject, forProjects: ReadonlyCollection<string>, seenProjects: Set<NormalizedPath>) {
        ...
        for (const child of children) {
            ...
            const { project: childProject } = this.getNewOrUpdatedConfiguredProject(
                configFileName,
